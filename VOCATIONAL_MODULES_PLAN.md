...
**Edge case:** One module might have multiple exams (e.g. quiz + project). Rule: module is passed if **any** exam for that module is passed? Or **all** exams for that module? — _Assume: module passed = at least one passed exam for that module._
...
- Programs can have `modules` during transition
